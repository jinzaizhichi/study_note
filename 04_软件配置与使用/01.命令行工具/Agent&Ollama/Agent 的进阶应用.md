...
因此，对于 Agent 应用来说，MCP 服务并不是“多多益善”的能力增强工具。它适用于那些确实需要真实的程序执行环境、跨系统交互或复杂工具编排的场景。如果仅仅是文本生成与结构化输出，过早引入协议层反而会造成过度工程化。总而言之，如果从能力分层的角度来看，MCP 服务属于“能力接入层”。它解决的是 LLM 的外部调用能力，而提示词则是用于控制 LLM 的单次推理行为与输出表现的。二者并不冲突，但也不在同一层级。理解这一点，才能避免将 MCP 误解为一种“高级提示词技巧”，从而在架构设计上做出错误决策。

> 顺带一提，如果读者想了解更多常用的 MCP 服务，以及它们在 Claude Code/Codex CLI 中的配置方法，也可以参考本文在“参考资料”一节中提供的视频教程：《15 款常用 MCP 服务的配置方法》。

## Skills 机制

在实际生产环境中，我们的许多任务场景都是高频度重复出现的；例如：将会议记录整理为结构化纪要，将长文档重写为对外发布版本，将需求说明转换为技术实现步骤，将代码进行安全审查与重构建议，等等。这些任务如果每次都依赖临时拼接提示词来完成，不仅效率低下，而且极易产生行为漂移。一次小的改动，可能导致输出风格或逻辑结构的整体变化。为了解决这些问题，Anthropic 公司于 2025 年 10 月 份提出了 Agent Skills 机制，该机制的核心思想是：

- 将某种“稳定的行为模式”抽象为一个可命名的能力单元；
- 将其输入输出结构进行明确约束；
- 并使其成为 Agent 可以直接调用的模块化组件；

从工程角度来看，这是一种“行为封装（Behavior Encapsulation）”的过程。如果说提示词是用于控制 LLM 的单次推理行为与输出表现，而 MCP 服务提供的是 LLM 的外部接入能力，那么 Anthropic 公司提出 Agent Skills 机制要解决的就是 LLM 行为逻辑的可复用性问题了。换言之，我们在 Agent 应用中引入 Skills 机制时，主要的思考方向应该是：

> 如何将已经验证有效的“行为模式”固化下来，并使其能够被重复调用、版本管理与组合使用？
...
在表面上看，Skills 与“提示词模板”似乎并无本质区别。它们都包含：
...
提示词模板通常只是自然语言文本的拼接，其结构往往隐含在文本描述之中。而 Skills 机制则会：
...
从这个角度来看，Skills 更像是 Agent 的“内部 API”，而非简单的文本工具。

### 3. 维护成本的差异

随着系统复杂度提升，如果所有行为逻辑都依赖提示词堆叠，那么系统提示会变得越来越庞大，修改风险也随之增加。

而通过 Skills 将高频行为模块化，可以：
